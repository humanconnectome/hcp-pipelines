...
class OneSubjectJobSubmitter(one_subject_job_submitter.OneSubjectJobSubmitter):
    ...
    def create_get_data_job_script(self):
        ...
        script.write("-type d -path '" + WORKING_DIR + "/" + SUBJECT_SESSION + "/unprocessed/[rt]fMRI_*' -exec rm -rf '{}' \;\n")
    ...
    def create_clean_data_script(self):
        ...
        script.write(WORKING_DIR + "/" + SUBJECT_SESSION + "/ProcessingInfo\n")
        script.write("cp " + WORKING_DIR + "/" + SUBJECT_SESSION + "/subjects/" + SUBJECT_SESSION + "/subject_hcp.txt ")
        script.write(WORKING_DIR + "/" + SUBJECT_SESSION + "/ProcessingInfo/processing\n")
        script.write("cp " + WORKING_DIR + "/" + SUBJECT_SESSION + "/subjects/" + SUBJECT_SESSION + "/hcpls/hcpls2nii.log ")
        script.write(WORKING_DIR + "/" + SUBJECT_SESSION + "/ProcessingInfo/processing\n")
        script.write("find " + WORKING_DIR + "/" + SUBJECT_SESSION)
        script.write(' -not -path "' + WORKING_DIR + "/" + SUBJECT_SESSION + "/" + 'T1w/*"')
        script.write(' -not -path "' + WORKING_DIR + "/" + SUBJECT_SESSION + "/" + 'ProcessingInfo/*"')
        script.write(' -not -path "' + WORKING_DIR + "/" + SUBJECT_SESSION + "/" + 'Diffusion/*"')
        script.write(" -delete")
        script.write("\n")
    ...
    def create_process_data_job_script(self):
        ...
        with open(script_name, "w") as script:
            script.write("#PBS -l nodes=" + PBS_NODES + ":ppn=" + PBS_PPN + ":gpus=1:K20x,walltime=" + WALLTIME_LIMIT_HOURS + ":00:00,mem=" + MEM_LIMIT_GBS + "gb\n")
            script.write("#PBS -o " + WORKING_DIR + "\n")
            script.write("#PBS -e " + WORKING_DIR + "\n")
            script.write("\n")
            script.write("module load cuda-9.1\n")
            script.write("module load " + SINGULARITY_CONTAINER_VERSION + "\n")
            script.write("\n")
            script.write("singularity exec --nv -B " + XNAT_PBS_JOBS_CONTROL + ":/opt/xnat_pbs_jobs_control," + XNAT_PBS_JOBS_ARCHIVE_ROOT + "," + SINGULARITY_BIND_PATH + "," + GRADIENT_COEFFICIENT_PATH + ":/export/HCP/gradient_coefficient_files " + SINGULARITY_CONTAINER_PATH + " " + RUN_QUNEX_SCRIPT + " \\\n")
            ...
            script.write("  --hcppipelineprocess=DiffusionPreprocessing\n")
            os.chmod(script_name, stat.S_IRWXU | stat.S_IRWXG)
