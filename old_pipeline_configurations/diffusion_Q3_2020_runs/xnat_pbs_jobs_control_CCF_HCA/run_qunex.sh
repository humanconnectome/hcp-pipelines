...
echo "HCPpipelineProcess - $PCPpipelineProcess"
if [ "${HCPpipelineProcess}" == "MultiRunIcaFixProcessing" ] || [ "${HCPpipelineProcess}" == "MsmAllProcessing" ] || [ "${HCPpipelineProcess}" == "DiffusionPreprocessing" ] ; then
	...
	if [ "${HCPpipelineProcess}" == "DiffusionPreprocessing" ] ; then
		cat /opt/xnat_pbs_jobs_control/batch.txt.tmpl | grep "^[0-9][0-9]*:" >> $BatchFile
	else
