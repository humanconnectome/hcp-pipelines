...
#FS_EXTRA_RECONALL=$1
#echo "FS_EXTRA_RECONALL=$FS_EXTRA_RECONALL"
#FsExtraReconall=`opts_GetOpt "--fs-extra-reconall" $@`
#echo "FsExtraReconall=${FsExtraReconall}"
#exit
...
## Brief sleep to make sure everything done here on out is reported as "newer"
sleep 2
log_Msg "PBS_JOBID: ${PBS_JOBID}" > ${filename}
log_Msg "PBS execution node: $(hostname)" >> ${filename}

if [ "${HCPpipelineProcess}" == "StructuralPreprocessingHandEdit" ]; then
	...
	######################### hcp2
		#--hcp_fs_extra_reconall='-autorecon2-wm|-autorecon3'
	${QUNEXCOMMAND} hcp2 \
		...
		--cores="${cores}" \
		--nprocess="0" \
		--hcp_fs_existing_subject="TRUE" \
		--hcp_fs_extra_reconall="$FsExtraReconall"
