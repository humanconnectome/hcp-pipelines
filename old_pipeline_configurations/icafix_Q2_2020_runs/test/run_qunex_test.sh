...
cat $scriptPath/batch.txt.tmpl | sed -e "s/@@@Subject@@@/${Subject}/g" -e "s/@@@SubjectPart@@@/${SubjectPart}/g" -e "s/@@@StudyFolder@@@/${StudyFolderRepl}/g" > $BatchFile
...
if [ "${HCPpipelineProcess}" == "StructuralPreprocessing" ]; then
	...
	######################### hcp_ReApplyFix
	echo ${QUNEXCOMMAND} hcp_ReApplyFix \
		--sessions="${StudyFolder}/processing/batch.txt" \
		--subjectsfolder="${StudyFolder}/subjects" \
		--overwrite="${Overwrite}" \
